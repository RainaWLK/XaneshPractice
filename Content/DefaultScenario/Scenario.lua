do
	...
	XANESHPRACTICE.DEFAULTSCENARIO.Scenario=XANESHPRACTICE.inheritsFrom(super)
	local class=XANESHPRACTICE.DEFAULTSCENARIO.Scenario
	
	XANESHPRACTICE.DEFAULTSCENARIO.PauseButtonWarning=false
	...
	function class:Populate()
		...
		local button
		for i=1,5 do
			button=XANESHPRACTICE.Button.new()
			...
			button.displayobject.drawable:SetScript("OnClick",function(self,button,down)
					if(down)then 
						...
						XANESHPRACTICE.DEFAULTSCENARIO.Layout.SetObelisks(scenario,i)
						self.xaneshstandstilltimer=0
		...
		local label
		label=XANESHPRACTICE.Label.new()
		...
		local guidelines=XANESHPRACTICE.DEFAULTSCENARIO.Guidelines1.new()
		guidelines:Setup(self.game.environment_gameplay)
		guidelines.position={x=500,y=200,z=0}
		self.guidelines1=guidelines
		
		local guidelines=XANESHPRACTICE.DEFAULTSCENARIO.Guidelines2.new()
		guidelines:Setup(self.game.environment_gameplay)
		guidelines.position={x=500,y=200,z=0}
		...
		--mob=XANESHPRACTICE.Mob.new()
		player=XANESHPRACTICE.PlayerCharacter.new()
		...
		-- local mobclickzone=XANESHPRACTICE.MobClickZone.new()
		-- mobclickzone:Setup(self.game.environment_gameplay,mob)		
		-- local nameplate=XANESHPRACTICE.Nameplate.new()
		...
		local controller=XANESHPRACTICE.PlayerController.new()
		...
		for i=1,0 do
			...
			if(not XANESHPRACTICE.PTR) then mob.displayobject.drawable:SetModelByFileID(1) end
...
--print("TOC:",XANESHPRACTICE.TOCVersion)
		-- mob=XANESHPRACTICE.Mob.new()		
		-- mob:Setup(self.game.environment_gameplay)		
		...
		obj=XANESHPRACTICE.WoWObject.new()
		...
		obj=XANESHPRACTICE.WoWObject.new()
		obj:Setup(self.game.environment_gameplay)		
		--if(not XANESHPRACTICE.PTR) then obj.displayobject.drawable:SetModelByFileID(1) end
		...
		----- WORLD MARKERS ------------------------------------------------
		obj=XANESHPRACTICE.WoWObject.new();obj:Setup(self.game.environment_gameplay)		
		obj.displayobject.drawable:SetModelByFileID(456041)	-- raid_ui_fx_red
		obj.position={x=-18,y=-18,z=0}
		obj.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.ProjectileSpawnCustomDuration)
		...
		xanesh.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.PriReadySpellCast)		
		...
		--local poly=XANESHPRACTICE.Polygon.new()
		...
		obj.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.Stun)
		obj=XANESHPRACTICE.WoWObject.new()
		obj:Setup(self.game.environment_gameplay)		
		obj.displayobject.drawable:SetModelByFileID(3177003)	--8fx_headcultist_voidbarrier_state		
		obj.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.ProjectileSpawn500ms)
		...
		-- --obj.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.ProjectileLoop)
		----------------------------------------------------------------------------------------------
		...
		-- mob=XANESHPRACTICE.PlayerCharacter.new()
		-- mob:Setup(self.game.environment_gameplay)		
		-- mob.position.x=4
	...
	function class:Step(elapsed)
		...
		if(self.soakstatus==1)then
			obj=XANESHPRACTICE.DEFAULTSCENARIO.GenericVoidNovaHigh.new()
			obj:Setup(self.game.environment_gameplay)		
		...
		elseif(self.soakstatus==2)then
			...
			obj=XANESHPRACTICE.DEFAULTSCENARIO.VoidRitualPortalProjectile.new()
			obj:Setup(self.game.environment_gameplay)
			obj.scenario=self
			obj.position={x=self.xanesh.position.x,y=self.xanesh.position.y,z=0}
			obj.portalclass=XANESHPRACTICE.DEFAULTSCENARIO.StartPortal
			...
			obj.scenario=self
			obj.position={x=self.xanesh.position.x,y=self.xanesh.position.y,z=0}			
			obj.portalclass=XANESHPRACTICE.DEFAULTSCENARIO.GoalPortal
			obj.destination={x=goalportalposition.x,y=goalportalposition.y,z=4}
			obj.orientation_displayed.yaw=math.atan2(obj.destination.y-obj.position.y,obj.destination.x-obj.position.x)			
			self.soakstatus=0
			--self.buttonphase=4
			obj=XANESHPRACTICE.DEFAULTSCENARIO.VoidwokeState.new()
			obj:Setup(self.game.environment_gameplay)				
	...
	function class:DeleteGoalPortal()
		...
		for i=1,#self.voidwokestates do
			...
			voidwokestate.animationmodule:PlayAnimation(XANESHPRACTICE.AnimationList.ProjectileDespawnCustomDuration)
		end
		self.voidwokestates={}
	end
	...
	function class:SpawnVoidRitualSoaks()
		...
		local obj
		for i=1,3 do
			obj=XANESHPRACTICE.DEFAULTSCENARIO.VoidRitualSoak.new()
			obj:Setup(self.game.environment_gameplay)		
			...
			if(self.useghosts)then
				local ghost=XANESHPRACTICE.DEFAULTSCENARIO.PlayerGhost.new()
