...
XANESHPRACTICE.Key = {}
XANESHPRACTICE.Key.__index = XANESHPRACTICE.Key

function XANESHPRACTICE.Key:new()
	local self={}
	setmetatable(self, XANESHPRACTICE.Key)	
...
function XANESHPRACTICE.Key:Setup(keys,keycode)
...
function XANESHPRACTICE.Key:SetKey(state)
	self.current=state
	if(state==true)then
		self.pressed=true
	else
...
function XANESHPRACTICE.Key:SetMouseWheel(delta)
	self.pressed=delta
	if(self.map)then
