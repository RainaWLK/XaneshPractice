do
	...
	XANESHPRACTICE.Camera = XANESHPRACTICE.inheritsFrom(super)
	local class=XANESHPRACTICE.Camera

	function class:SetCustomInfo()
		XANESHPRACTICE.GameObject.SetCustomInfo(self)
	...
	function XANESHPRACTICE.Camera:CreateFrameLayers()
	--TODO: framelayers should be recycled between our 3+ varieties of Camera
		...
		self.nameplateframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(self.topframelevel)
		...
		self.ysortframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		-- telegraphs, voidzones, etc are placed below the first ysorted object
		self.movementarrowframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		self.telegraphframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		self.voidzoneframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		self.targetcircleframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		self.shadowframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		-- arena shape needs 2 layers (one for border, one for fill)
		layer=layer-1
		layer=layer-1
		self.arenaborderframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)
		layer=layer-1
		self.floorframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)	
		layer=layer-1
		self.skybox2framelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)		
		layer=layer-1
		self.skyboxframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(layer)	
		
		self.defaultframelayer=XANESHPRACTICE.FrameLayer.QuickSetup(100)
	end


	function XANESHPRACTICE.Camera:Step(elapsed)
	...
	function XANESHPRACTICE.Camera:MoveToFocus(elapsed)
	--TODO: smooth scrolling
		...
		local extra=XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(5)
		xmin=xmin-extra
		ymin=ymin-extra
		xmax=xmax+extra
		ymax=ymax+extra
	...
	function XANESHPRACTICE.Camera:PursueTargetZoom(elapsed)
	...
	function XANESHPRACTICE.Camera:PursueTargetCoords(elapsed)
		...
		if(self.snaptotarget)then
			--if(distsqr>XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(7*3)*elapsed)then
			if(distsqr>XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(7*5)*elapsed)then
				self.snaptotarget=false
			else
		...
		if(not self.snaptotarget)then
			--if(distsqr<=XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(7)*elapsed)then
			if(distsqr<=XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(7*2)*elapsed)then
		...
		-- local unitvectorx=0
		-- local unitvectory=0
		-- local movespeed=XANESHPRACTICE.Mobile_ConvertYardsToTGUnits(14)
	...
	function XANESHPRACTICE.Camera:AdjustZoomViaScrollWheel(delta)
	...
	function XANESHPRACTICE.Camera:SetScrollAndZoom()
		...
		-- SetScale affects 3D models in an unexpected way, but we fix them in Drawable_CharacterModel
			--TODO NEXT: it also affects non-character models in an even more unexpected way
		self.environment.frame:SetScale(self.zoom)	
	end

	function XANESHPRACTICE.Camera:ArrangeAllGameObjects(objs)
		...
		for i=1,#objs,1 do
			...
			-- if(obj.drawable.frame.name=="XANESHPRACTICE.ReusableFrame0")then
	...
	function XANESHPRACTICE.Camera:GetScrollAmounts()
	...
	-- NOT USED
	function XANESHPRACTICE.Camera:SetObjectVisibility(objs)
		if(true)then return end
	...
	function XANESHPRACTICE.Camera:LocalCoordsToCameraCoords(x,y)
	...
	function XANESHPRACTICE.Camera:ConvertObjectCoordsToMouseoverCoords(x,y,obj)
	...
	function XANESHPRACTICE.Camera:WindowCoordsToCameraCoords(x,y)
	...
	function class:Draw()
		...
		self.position.x,self.position.y,self.position.z=XANESHPRACTICE.MatrixMultiplicationRotation2(-self.cdist,0,0,0,self.orientation.pitch,self.orientation.yaw)
