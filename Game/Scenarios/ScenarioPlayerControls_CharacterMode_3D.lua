...
do
	local super=XANESHPRACTICE.ScenarioPlayerControls
	XANESHPRACTICE.ScenarioPlayerControls_CharacterMode_3D = XANESHPRACTICE.inheritsFrom(super)
	local class=XANESHPRACTICE.ScenarioPlayerControls_CharacterMode_3D
	...
	function class:GetPlayerInput(elapsed)
		...
		if(self.environment.game.keys.lmouse.pressed or self.environment.game.keys.rmouse.pressed)then
			--TODO NEXT: if both mouse buttons are pressed at the same time, adjust camera angle without waiting for mousemove
			mousepressed=true
		end
			...
			if(self.environment.game.keys.rmouse.current)then
				...
				if(player)then
					if((xdelta~=0 or ydelta~=0) or mousepressed)then
						...
						-- else
							-- aura=player.combat:ApplyAuraByClass(XANESHPRACTICE.Aura_Facing_CameraTest,player.combat)					
						-- end
		...
		if(vectorx==0 and vectory==0)then
			...
			vectorx,vectory=XANESHPRACTICE.Transform_Rotate2D(vectorx,vectory,facing,0,0)
