...
XANESHPRACTICE.ScenarioPlayerControls = XANESHPRACTICE.inheritsFrom(XANESHPRACTICE.GameObject)

--TODO: move scenario back to game?  it needs to keep track of multiple environments
function XANESHPRACTICE.ScenarioPlayerControls:Setup(environment,scenario)
	XANESHPRACTICE.GameObject.Setup(self,environment,scenario)
	self.scenario=scenario	
end

-- TODO: base SPC shouldn't have selectionrectangle, as charactermode inherits from it
	-- need new SPC_RTSMode class
function XANESHPRACTICE.ScenarioPlayerControls:CreateAssociatedObjects()	
-- dubious
	-- local selection=XANESHPRACTICE.SelectionRectangle.new()
...
function XANESHPRACTICE.ScenarioPlayerControls:SetPlayer(player)
	self.player=player
end


function XANESHPRACTICE.ScenarioPlayerControls:GetPlayerInput(elapsed)
	...
	if(self.environment.game.keys.rmouse.pressed)then
		...
		for i=1,#environment_gameplay.mobiles,1 do
			...
			if(obj.selected and obj.faction==XANESHPRACTICE.Factions.PLAYER)then
...
function XANESHPRACTICE.ScenarioPlayerControls:FilterSelectableObjectList(player,objs)
	return objs
end
...
function XANESHPRACTICE.ScenarioPlayerControls:CreateSpellButtonsFromSelectedUnits()
	for k,v in pairs(XANESHPRACTICE.Factions) do
	...
	XANESHPRACTICE.RemoveDuplicates(spells)
	...
	for k,faction in pairs(XANESHPRACTICE.Factions) do		
